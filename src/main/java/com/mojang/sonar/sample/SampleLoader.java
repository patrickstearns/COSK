...
public class SampleLoader
{
    ...
    public static SonarSample loadSample(String resourceName) throws UnsupportedAudioFileException, IOException{
    	...
    	byte[] d = rip(new FileInputStream(new File(resourceName)));
    	//used to be:        byte[] d = rip(SampleLoader.class.getResourceAsStream(resourceName));
        AudioInputStream ais = AudioSystem.getAudioInputStream(new ByteArrayInputStream(d));
        return buildSample(rip(ais), ais.getFormat());
    }

    public static SonarSample loadSampleFromStream(InputStream stream) throws UnsupportedAudioFileException, IOException {
        byte[] d = rip(stream);
        AudioInputStream ais = AudioSystem.getAudioInputStream(new ByteArrayInputStream(d));
        return buildSample(rip(ais), ais.getFormat());
    }
