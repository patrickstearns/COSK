...
public class EdgedPainter implements Painter {

    private int ob, bb;

    public EdgedPainter(int bb, int ob){
        this.bb = bb;
        this.ob = ob;
    }
    ...
    public void paint(AreaGroup areaGroup, int x, int y) {
        if (!isPaintedTile(areaGroup.getBlock(x, y, Area.Layer.Lower).blockId)){
            ...
            if (isPaintedTile(areaGroup.getBlock(x-1, y, Area.Layer.Lower).blockId))
                paintTile(areaGroup, x - 1, y, newTileId(areaGroup, x - 1, y));
            if (isPaintedTile(areaGroup.getBlock(x+1, y, Area.Layer.Lower).blockId))
                paintTile(areaGroup, x + 1, y, newTileId(areaGroup, x + 1, y));
            if (isPaintedTile(areaGroup.getBlock(x, y-1, Area.Layer.Lower).blockId))
    ...
    private int newTileId(AreaGroup areaGroup, int x, int y){
        ...
        if (x > 0 && y > 0) upLeftPainted = isPaintedTile(areaGroup.getBlock(x, y-1, Area.Layer.Lower).blockId);
        if (x > 0 && y < areaGroup.getHeight()-1) downLeftPainted = isPaintedTile(areaGroup.getBlock(x, y+1, Area.Layer.Lower).blockId);
        if (x < areaGroup.getWidth()-1 && y < 0) upRightPainted = isPaintedTile(areaGroup.getBlock(x-1, y, Area.Layer.Lower).blockId);
        if (x < areaGroup.getWidth()-1 && y < areaGroup.getHeight()-1) downRightPainted = isPaintedTile(areaGroup.getBlock(x+1, y, Area.Layer.Lower).blockId);
