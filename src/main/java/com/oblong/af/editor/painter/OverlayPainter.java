...
public class OverlayPainter implements Painter {

    private boolean animated, drawsOnMainLayer;
    private int ob, bb;

    public OverlayPainter(int bb, int ob, boolean animated, boolean drawsOnMainLayer){
        this.bb = bb;
        this.ob = ob;
        this.animated = animated;
        this.drawsOnMainLayer = drawsOnMainLayer;
    }
    ...
    public void paint(AreaGroup areaGroup, int x, int y) {
        if (!isPaintedTile(areaGroup.getBlock(x, y, Area.Layer.Main).blockId)){
            ...
            if (isPaintedTile(areaGroup.getBlock(x-1, y-1, Area.Layer.Main).blockId))
                paintTile(areaGroup, x-1, y-1, newTileId(areaGroup, x-1, y-1));
    ...
    private void paintTile(AreaGroup areaGroup, int x, int y, int b){
        if (b == bb && !animated && !drawsOnMainLayer){
            areaGroup.setBlock(x, y, Area.Layer.Lower, b);
            areaGroup.setBlock(x, y, Area.Layer.Main, AreaGroup.DEFAULT_BLOCK);
        }
        else{
            int blockThere = areaGroup.getBlock(x, y, Area.Layer.Main).blockId;
            if (blockThere != 0){
                Painter painter = PainterFactory.findPainterForBlock(blockThere);
                if (painter != null && painter != this) areaGroup.setBlock(x, y, Area.Layer.Lower, painter.getBaseBlockId());
            }
    ...
    public int getBaseBlockId(){ return bb; }
    ...
    public boolean isPaintedTile(int b){
        ...
        return ((b == bb) ||
                (b >= ob && b < ob+5*m) ||
                (b >= ob+16 && b < ob+16+5*m) ||
                (b >= ob+16*2 && b < ob+16*2+5*m) ||
                (b >= ob+16*3 && b < ob+16*3+5*m));
    }
    ...
    private int newTileId(AreaGroup areaGroup, int x, int y){
        ...
        //four connections
            ...
            //strips off in every direction
            if (!upLeftPainted && !upRightPainted && !downLeftPainted && !downRightPainted){
                xo = 0;
                yo = 3;
            }
            else if (!upLeftPainted && upRightPainted && !downLeftPainted && !downRightPainted){
                xo = 4;
                yo = 0;
            }
            else if (!upLeftPainted && !upRightPainted && !downLeftPainted && downRightPainted){
            ...
            else useDefault = true; //use the basic one
        }

        int b = bb;
        if (animated) xo *= 3;
        if (!useDefault) b = ob+xo+yo*16;
        return b;
    }
