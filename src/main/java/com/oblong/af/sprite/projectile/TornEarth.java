...
public class TornEarth extends Prop {

    public static TornEarth createEarthRend(AreaScene scene, Prop parent, double heading){
        float x = parent.getX()+(float)(Math.cos(heading)*16);
        float y = parent.getY()-(float)(Math.sin(heading)*16)+8; //8 to make look coming out at feet
        return new TornEarth(scene, parent, heading, x, y, 64, 0);
    }
    ...
    public TornEarth(AreaScene scene, Prop parent, double heading, float x, float y, int timeToLive, int index){
        super("torn earth", scene, 0, 9);
        ...
        setImpactDamageAttributes(new DamageAttributes(2, Arrays.asList(Attribute.Earth, Attribute.Physical)));
        this.index = index;
    ...
    public void move(){
        ...
        if (index < maxIndex && getTick() == 2){
            double h = getHeading();
            h = h-0.3d+0.6d*Math.random();
            float nx = getX()+(float)(Math.cos(h)*12);
            float ny = getY()-(float)(Math.sin(h)*12);
            getScene().addSprite(new TornEarth(getScene(), parent, h, nx, ny, timeToLive-1, index+1));
        }
