...
public class PoisonCloud extends Prop {

    private int timeToLive;
    private Prop parent;
    private boolean persists;

    public PoisonCloud(AreaScene scene, int timeToLive, Prop parent, boolean persists) {
        super("poison cloud", scene, 0, 8);
        ...
        setImpactDamageAttributes(new DamageAttributes(0, Arrays.asList(Attribute.Poison)));
    ...
    protected void collided(Prop with){
        if (with != parent && !(with instanceof PoisonCloud)){
            if (!persists) die();
    ...
    public boolean isCollidableWith(Prop prop){
        ...
        if (parent instanceof GigapedeSegment && prop instanceof GigapedeSegment) return false;
